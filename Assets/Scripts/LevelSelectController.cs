...
public class LevelSelectController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(adjustedMousePos.x > .75f)
        {
            ...
            {
                theCam.position += theCam.right * moveSpeed * Time.deltaTime * 0.5f;
            }
        ...
        if (adjustedMousePos.y > .75f)
        {
            ...
            {
                theCam.position += theCam.forward * moveSpeed * Time.deltaTime * 0.5f;
            }
        ...
        theCam.position = new Vector3(Mathf.Clamp(theCam.position.x, minPos.position.x, maxPos.position.x), 
            theCam.position.y, 
            Mathf.Clamp(theCam.position.z, minPos.position.z, maxPos.position.z));
    }
}
